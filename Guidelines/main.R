...
par(mfrow=c(1,1))
library(glmnet)
source("C:\\Users\\its_t\\Documents\\CUNY\\Spring 2020\\9890 - Statistical Learning for Data Mining\\HW\\HW 3\\plotfuns.R")
load("C:\\Users\\its_t\\Documents\\CUNY\\Spring 2020\\9890 - Statistical Learning for Data Mining\\HW\\HW 3\\bstar.Rdata")
...
a.rid = glmnet(X.tr, y.tr, alpha = 0, lambda = 3) # for the ridge regression solutions, use alpha=0
a.las = glmnet(X.tr, y.tr, alpha = 1, lambda = 0.056) # for the lasso solutions, use alpha=1

plot.image(a.rid$beta)
plot.image(a.las$beta)


a.rid = glmnet(X.tr, y.tr, alpha = 0, lambda = 13) # for the ridge regression solutions, use alpha=0
a.las = glmnet(X.tr, y.tr, alpha = 1, lambda = 0.144) # for the lasso solutions, use alpha=1
par(mfrow=c(1,2))
plot.image(a.rid$beta)
plot.image(a.las$beta)
